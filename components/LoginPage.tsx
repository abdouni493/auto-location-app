...
const LoginPage: React.FC<LoginPageProps> = ({ onLogin, lang, onLanguageToggle, storeName = 'DriveFlow', logo = '' }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isAdmin) {
        ...
        // Try to login as worker - check workers table
          ...
          <div className="p-10 md:p-14">
            <div className="flex justify-center mb-12">
              <div className="relative group">
                ...
                <div className="relative w-28 h-28 bg-gradient-to-br from-blue-600 to-red-600 rounded-[2.5rem] flex items-center justify-center text-white text-6xl font-black shadow-2xl overflow-hidden">
                  {logo ? <img src={logo} className="w-full h-full object-cover" alt="Logo" /> : storeName.charAt(0).toUpperCase()}
                </div>
              </div>
            </div>

            <div className="text-center mb-12">
              <h1 className="text-4xl font-black text-gray-900 tracking-tight mb-3">{storeName}</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="space-y-3">
                <label className={`block text-[10px] font-black text-gray-900 tracking-widest uppercase px-2 ${isRtl ? 'text-right' : ''}`}>
                  {lang === 'fr' ? "Email ou Utilisateur" : "البريد الإلكتروني أو اسم المستخدم"}
                </label>
                <div className="relative group">
